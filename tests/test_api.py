...
def test_admin_reconcile_events_requires_admin_and_returns_summary(tmp_path, monkeypatch):
    client, store = make_client(tmp_path)
    admin = store.create_worker("boss", is_admin=True)
    ...
    class Result:
        ...
        skipped_existing_reconstructed = 0
        skipped_actor_filter = 1
        skipped_unresolved_actor = 0

    def fake_reconcile(store_arg, decomp_arg, *, actors, apply):
        assert store_arg is store
        assert actors == {"JeBobs"}
        assert apply is False
        return Result()

    monkeypatch.setattr(api, "reconcile_review_events_from_decomp", fake_reconcile)

    assert client.post("/admin/reconcile-events", json={"actors": ["JeBobs"]}).status_code == 401
    response = client.post(
        "/admin/reconcile-events",
        headers={"X-Work-Token": admin["token"]},
        json={"actors": ["JeBobs"], "apply": False},
    )
